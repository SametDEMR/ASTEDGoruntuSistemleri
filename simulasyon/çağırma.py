...
import time
import random
from kenardefo import *
from lekeekle import *
from kenarveleke import *
from TESPİT import *

image_path = "b.png"
save_counter = 0

clean_img = cv2.imread(image_path)  # Orijinal temiz görüntü bellekte

while True:
    choice = random.choice(["distort", "stains", "kenar+leke", "none"])

    if choice == "distort":
        img = inward_distort_image(image_path)
    elif choice == "kenar+leke":
        img = distort_and_add_stains(image_path)
    elif choice == "stains":
        img = add_stains_and_particles(image_path)
    else:
        img = clean_img.copy()

    if img is None:
        print("Görüntü işlenemedi, çıkılıyor.")
        break

    # Bozukluk tespiti (orijinal temiz ve yeni bozuk img)
    detected_img = detect_defects_by_diff_images(clean_img, img,
                                                 threshold=30,
                                                 min_area=.01,
                                                 scale_percent=100,
                                                 border_margin=20)

    # Görüntüleri yan yana koyup göstermek istersen:
    combined = cv2.hconcat([img, detected_img])
    cv2.imshow("Effect and Defect Detection", combined)

    # Kaydetmek istersen
    save_path = f"output_{save_counter}.png"
